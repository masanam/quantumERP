...
class customers_app extends application 
{
	...
	{
		$this->application("orders", _($this->help_context = "Penjualan"));
	
		$this->add_module(_("Transaksi"));
...
$this->add_lapp_function(0, _("&Find Outstanding Sales Order"),
			"sales/inquiry/sales_orders_view.php?OutstandingOnly=1", 'SA_SALESDELIVERY', MENU_TRANSACTION);			
		...
		$this->add_lapp_function(0, _("&Find Outstanding Delivery Order"),
			"sales/inquiry/sales_deliveries_view.php?OutstandingOnly=1", 'SA_SALESINVOICE', MENU_TRANSACTION);
		...
		$this->add_module(_("Pencarian & Laporan"));
		...
		$this->add_module(_("Pemeliharaan"));
